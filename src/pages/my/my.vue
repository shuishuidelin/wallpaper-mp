<template>
  <view class="content">
    <view class="top row">
      ...
      <view class="name" @click="startLogin">{{ userName }}</view>
    </view>
    <view class="panel">
      <!-- <view class="item row">
...
export default defineComponent({
  ...
  setup() {
    ...
    function launchApp() {
      uni.setClipboardData({
        ...
        success: () => {
          ...
          uni.showToast({
            title:
              '复制成功,请粘贴到iOS手机自带的Safari浏览器打开,按照说明安装...',
            icon: 'none',
          })
        },
      })
    ...
    function startLogin() {
      console.log('=========login')
      if (store.state.username != '去登录') {
        console.log('已经获取到用户信息不用再次获取....')
        return
      }
      uni.getUserInfo({
        success(res) {
          console.log('ksuserInfo', res.userInfo)
          store.commit('editUsername', res.userInfo.nickName || '去登录')
          store.commit('editHeadImg', res.userInfo.avatarUrl || '')
          userName.value = res.userInfo.nickName
          headImg.value = res.userInfo.avatarUrl
        },
      })
    }

    let userName = ref(store.state.username ? store.state.username : '去登录')
    let headImg = ref(store.state.headImg ? store.state.headImg : '')
